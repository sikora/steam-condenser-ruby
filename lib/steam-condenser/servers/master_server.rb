...
module SteamCondenser
  ...
  module Servers
    ...
    #
    # An intance of this class can be used much like Steam's server browser to
    # get a list of available game servers, including filters to narrow down
    # the search results.
    #
    ...
    class MasterServer
      ...
      #       time.
      # @param [Fixnum] region_code The region code to specify a location of
      #        the game servers
      # @param [String] filters The filters that game servers should match
      # @param [Boolean] force Return a list of servers even if an error
      #        occured while fetching them from the master server
      # @raise [Error::Timeout] if too many timeouts occur while querying the
      #        master server
      # @return [Array<Array<String>>] A list of game servers matching the
      #         given region and filters
      ...
      # @return [Array<SteamPacket>] Zero or more reply packets from the
      #         server. Zero means either the heartbeat was accepted by the
